...
# Stackored Pull Script - Pulls all images sequentially to avoid rate limits

readonly SCRIPT_PATH="$(readlink -f "${BASH_SOURCE[0]}")"
readonly STACKORED_ROOT="$(cd "$(dirname "$SCRIPT_PATH")/../.." && pwd)"

# Docker Compose dosya yolları
...
if [ -z "$services" ]; then
    echo "❌ No services found. Run 'stackored generate' first."
    exit 1
fi
...
echo ""
echo "✅ Pull complete! Now run: stackored up"
